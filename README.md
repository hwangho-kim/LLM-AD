...
공정 컨텍스트 제공: "언제, 어디서, 어떻게" 공정이 진행되었는지에 대한 정보를 담습니다.

wafer_id, equipment_id, recipe_id, final_yield

FDC 데이터

fdc_data

공정 상태 데이터: 공정 중 장비 내부 센서(온도, 압력 등)의 시계열 데이터를 제공합니다.

pressure, rf_power, gas_flow, temperature의 시계열
...
특정 시점에 센서 값이 순간적으로 급등/급락하는 현상.

RF 파워 불안정

[이미지: 위로 뾰족하게 튀는 스파이크 그래프]

Variance

공정 중 센서 값의 변동성(분산)이 비정상적으로 커지는 현상.

가스 유량 제어 불안

[이미지: 정상보다 위아래로 진동 폭이 큰 그래프]

Drift

센서 값이 시간이 지남에 따라 서서히 증가하거나 감소하는 현상.

챔버 온도 변화

[이미지: 오른쪽으로 완만하게 상승하는 그래프]
...
_mean, _std, _max, _min, _skew, _kurt

평균, 표준편차, 최대/최소, 왜도, 첨도

시계열의 기본적인 분포와 통계적 특성 파악

도메인 특화

_stable_std

안정 구간(20%~80%)의 표준편차: 공정 핵심 단계의 안정성 정밀 평가
...
곡선 아래 면적(Area Under Curve): 공정 중 가해진 총 에너지/가스량
...
선형 추세(기울기): 공정 전반의 점진적 변화(드리프트) 포착
...
피크(Spike) 개수: 평균에서 3-시그마를 벗어나는 이상 신호의 빈도
...
[이미지: 입력층, 인코더, 잠재 공간, 디코더, 출력층으로 구성된 오토인코더 구조도]
...
autoencoder.fit() 함수로 정상 데이터만을 사용하여 모델을 학습시킵니다.

비지도 학습(Unsupervised Learning)

임계치 설정

학습된 정상 데이터들의 복원 오류 중 상위 1%(99 분위수) 값을 임계치로 설정합니다.

통계 기반의 합리적 기준 설정

성능 평가

classification_report를 통해 정밀도(Precision), 재현율(Recall), F1-Score를 계산합니다.

정확도(Accuracy)가 아닌, 불균형 데이터에 적합한 지표 사용
...
[이미지: SHAP Force Plot 예시. base value에서 출발하여 여러 피처(빨간색/파란색)들이 예측값을 밀어내는 형태의 그래프]
...
[이미지: SHAP Summary Plot(beeswarm) 예시. Y축에는 피처 이름들이 중요도 순으로 나열되고, X축에는 각 점들이 SHAP 값에 따라 분포하는 그래프]
